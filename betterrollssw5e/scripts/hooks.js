	...
	*/
    static registerActorSheet( // SHEET CREATORS SHOULD NO LONGER USE THIS!
        ...
        Hooks.on(sheetString, (app, html, data) => {
            game.settings.get("betterrollssw5e", "rollButtonsEnabled")
                ? addItemSheetButtons(
                      app.object,
                      html,
                      data,
                ...
                : null;
            game.settings.get("betterrollssw5e", "diceEnabled")
                ? changeRollsToDual(app.object, html, data, params)
                : null;
        });
    }

    static registerItemSheet(sheetName) {
        // SHEET CREATORS SHOULD NO LONGER USE THIS!
        let sheetString = "render" + sheetName;
        Hooks.on(sheetString, (app, html, data) => {
            game.settings.get("betterrollssw5e", "diceEnabled")
	...
	*/
    static addItemContent(
        ...
        buttonContainer
            ...
            : null;
        game.settings.get("betterrollssw5e", "diceEnabled") && itemButton
            ? changeRollsToDual(actor, html, null, { itemButton: itemButton })
            : null;
    }
}
...
Hooks.on("renderChatMessage", (message, html, data) => {
    ...
    if ((!actor && !game.user.isGM) || actor?.permission != 3) {
        html.find(".hideSave").text(i18n("brsw5e.hideDC.string"));
    }
});

/*
Hooks.on("renderedMagicItems", (actor, html, data) => {
	if (window.MagicItems && game.settings.get("betterrollssw5e", "diceEnabled")) {
