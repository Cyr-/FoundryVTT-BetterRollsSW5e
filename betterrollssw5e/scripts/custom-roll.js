import { i18n, isAttack, isSave, getSave, isCheck } from "./betterrollssw5e.js";
...
export class CustomRoll {
	...
	*/
	static async rollMultiple(numRolls = 1, dice = "1d20", parts = [], data = {}, title = "", critThreshold, rollState, triggersCrit = false, rollType = "") {
		...
		const multiRoll = await renderTemplate("modules/betterrollssw5e/templates/red-multiroll.html", chatData);
	...
	static async eventToAdvantage(ev, itemType) {
		if (ev.shiftKey) {
		...
		} else if (game.settings.get("betterrollssw5e", "queryAdvantageEnabled")) {
			...
			return new Promise(resolve => {
				new Dialog({
					title: i18n("brsw5e.querying.title"),
					buttons: {
						disadvantage: {
							label: i18n("brsw5e.querying.disadvantage"),
							callback: () => resolve({adv:0, disadv:1})
						},
						normal: {
							label: i18n("brsw5e.querying.normal"),
							callback: () => resolve({adv:0, disadv:0})
						},
						advantage: {
							label: i18n("brsw5e.querying.advantage"),
	...
	static async fullRollSkill(actor, skill, params) {
		...
		const label = sw5e.skills[skill];
		const multiRoll = await CustomRoll.rollSkillCheck(actor, skl, params);

		const titleTemplate = await renderTemplate("modules/betterrollssw5e/templates/red-header.html", {
			item: {
		...
		const content = await renderTemplate("modules/betterrollssw5e/templates/red-fullroll.html", {
			title: titleTemplate,
			templates: [multiRoll]
		});
	...
	*/
	static async fullRollAttribute(actor, ability, rollType, params) {
		...
		if (rollType === "check") {
			...
			titleString = `${i18n(label)} ${i18n("brsw5e.chat.check")}`;
		} else if (rollType === "save") {
			multiRoll = await CustomRoll.rollAbilitySave(actor, ability, params);
			titleString = `${i18n(label)} ${i18n("brsw5e.chat.save")}`;
		}
...
*/
	...
	/**
	 * Updates the rollRequests based on the brsw5e flags.
	 */
	updateForPreset() {
		let item = this.item,
	...
	*/
	listProperties() {
		...
		switch(item.data.type) {
			...
			case "spell":
				// Spell attack labels
				data.damageLabel = data.actionType === "heal" ? i18n("brsw5e.chat.healing") : i18n("brsw5e.chat.damage");
	...
	async rollAttack(preArgs) {
		...
		let itemData = itm.data.data,
			...
			title = (this.config.rollTitlePlacement !== "0") ? i18n("brsw5e.chat.attack") : null,
			parts = [],
	...
	async rollDamage({damageIndex = 0, forceVersatile = false, forceCrit = false, bonus = 0, customContext = null}) {
		...
		else if (sw5e.damageTypes[damageType]) { titleString += i18n("brsw5e.chat.damage"); }
	...
	*/
	...
	async critRoll(rollFormula, rollData, baseRoll) {
		...
		if (itm.data.type === "weapon") {
			...
			catch(error) { savage = itm.actor.getFlag("sw5e", "savageAttacks"); }
		}
	...
	*/
	async rollOther(preArgs) {
		...
		// Title
		let titleString = i18n("brsw5e.chat.other"),
	...
	*/
	async saveRollButton({customAbl = null, customDC = null}) {
		...
		let saveLabel = `${i18n("brsw5e.buttons.saveDC")} ` + divHTML + ` ${sw5e.abilities[saveData.ability]}`;
		let button = {
			type: "saveDC",
	...
	async rollTool(preArgs) {
		...
		let itemData = itm.data.data,
			actorData = itm.actor.data.data,
			title = args.title || ((this.config.rollTitlePlacement != "0") ? i18n("brsw5e.chat.check") : null),
			parts = [],
			rollData = duplicate(actorData);
	...
	rollFlavor(preArgs) {
		...
		return {
			type: "flavor",
			html: `<div class="brsw5e-roll-label br-flavor">${args.text}</div>`
		};
	}
	...
	async configureSpell() {
	  ...
	  if ( slots === 0 || Number.isNaN(slots) ) {
				ui.notifications.error(game.i18n.localize("SW5E.SpellCastNoSlots"));
				return "error";
			}
			await actor.update({
	...
	async consumeCharge() {
		...
		if (hasUses && request.use && !request.quantity) {
			if (!current) { ui.notifications.warn(game.i18n.format("SW5E.ItemNoUses", {name: item.name})); return "error"; }
		}

		// Check for consuming quantity, but not uses
		if (request.quantity && !request.use) {
			if (!q) { ui.notifications.warn(game.i18n.format("SW5E.ItemNoUses", {name: item.name})); return "error"; }
		}

		// Check for consuming quantity and uses
		if (hasUses && request.use && request.quantity) {
			if (!current && q <= 1) { ui.notifications.warn(game.i18n.format("SW5E.ItemNoUses", {name: item.name})); return "error"; }
		}

		// Check for consuming charge ("Action Recharge")
		if (request.charge) {
			if (!recharge.charged) { ui.notifications.warn(game.i18n.format("SW5E.ItemNoUses", {name: item.name})); return "error"; }
		}
		...
		// Handle quantity and uses
			...
			if (remainingU < 1) {
				remainingQ -= 1;
				ui.notifications.warn(game.i18n.format("brsw5e.error.autoDestroy", {name: item.name}));
				if (remainingQ >= 1) {
