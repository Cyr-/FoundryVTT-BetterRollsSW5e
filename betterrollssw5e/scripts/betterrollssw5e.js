...
Hooks.on(`ready`, () => {
	...
	if (critText.includes("brsw5e.critString")) {
...
Hooks.on(`createOwnedItem`, (actor, itemData) => {
	game.settings.get("betterrollssw5e", "diceEnabled") ? ItemUtils.ensureFlags(game.actors.get(actor._id).items.get(itemData._id)) : null;
});
...
async function addButtonsToItemLi(li, actor, buttonContainer) {
	...
	switch (item.data.type) {
		...
		case 'consumable':
			...
			if (diceEnabled) {
				buttons.append(
					createButton({ content: i18n("brsw5e.buttons.roll"), action: "quickRoll" }),
					createButton({ content: i18n("brsw5e.buttons.altRoll"), action: "altRoll"})
				)
			};

			if (isAttack(item)) {
				buttons.append(
					createButton({ content: i18n("brsw5e.buttons.attack"), action: "attackRoll"})
				);
			}
			...
			if (isSave(item)) {
				...
				buttons.append(
					createButton({
						content: `${i18n("brsw5e.buttons.saveDC")} ${saveData.dc} ${sw5e.abilities[saveData.ability]}`,
						action: "save"
					})
				);
			}
			...
			if (itemData.damage.parts.length > 0) {
				buttons.append(
					createButton({ content: i18n("brsw5e.buttons.damage"), action: "damageRoll", value: "all" })
				);

				if (itemData.damage.versatile) {
					buttons.append(
						createButton({ content: i18n("brsw5e.buttons.verDamage"), action: "verDamageRoll", value: "all" })
					);
				}
			...
			if (itemData.formula.length > 0) {
				const otherString = contextEnabled && flags.quickOther.context || "brsw5e.settings.otherFormula";
		...
		case 'tool':
			...
			buttons.append(
				createButton({
					content: `${i18n("brsw5e.buttons.itemUse")} ${item.name}`,
					action: "toolCheck",
	...
	if (diceEnabled) {
		buttons.append(
			createButton({ content: i18n("brsw5e.buttons.info"), action: "infoRoll" })
		);
	}
	
	// Add default roll button
	buttons.append(
		createButton({ content: i18n("brsw5e.buttons.defaultSheetRoll"), action: "vanillaRoll" })
	);
	...
	buttons.find('button').click((ev) => {
		...
		if (diceEnabled) {
			...
			if (params.forceCrit) {
				fields.push([
					"flavor",
					{ text: `${game.settings.get("betterrollssw5e", "critString")}` }
				]);
			}
...
export function changeRollsToDual (actor, html, data, params) {
	...
	if (itemImage.length > 0) {
		...
		itemImage.click(async event => {
			...
			} else if (!game.settings.get("betterrollssw5e", "imageButtonEnabled")) {
...
export function BetterRolls() {
	...
	function vanillaRoll(actorId, itemId) {
		...
		if (!actor) { return ui.notifications.warn(`${i18n("brsw5e.error.noActorWithId")}`); }
		let item = actor.getOwnedItem(itemId);
		if (!item) { return ui.notifications.warn(`${i18n("brsw5e.error.noItemWithId")}`); }
		if (actor.permission != 3) { return ui.notifications.warn(`${i18n("brsw5e.error.noActorPermission")}`); }
		item.roll()
	};
	...
	function quickRoll(itemName) {
		...
		if (!actor) { return ui.notifications.warn(`${i18n("brsw5e.error.noSelectedActor")}`); }
		else if (!item) { return ui.notifications.warn(`${actor.name} ${i18n("brsw5e.error.noKnownItemOnActor")} ${itemName}`); }
	...
	function quickRollByName(actorName, itemName) {
		...
		if (!actor) { return ui.notifications.warn(`${i18n("brsw5e.error.noKnownActorWithName")}`); }
		let item = actor.items.find(i => i.name === itemName);
		if (!item) { return ui.notifications.warn(`${actor.name} ${i18n("brsw5e.error.noKnownItemOnActor")} ${itemName}`); }
		if (actor.permission != 3) { return ui.notifications.warn(`${i18n("brsw5e.error.noActorPermission")}`); }
		new CustomItemRoll(item, {event:event, preset:(isAlt(event) ? 1 : 0)}).toMessage();
	};
