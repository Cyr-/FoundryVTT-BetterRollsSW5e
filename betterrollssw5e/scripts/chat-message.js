...
class BetterRollsChatCard {
    ...
    async _setupDamageButtons() {
        ...
        const template = await renderTemplate(
            "modules/betterrollssw5e/templates/red-damage-overlay.html"
        );
        const dmgElements = html
            .find(".red-base-die")
            .parents(".dice-total")
            .toArray();
        const customElements = html
            .find("[data-type=custom] .red-base-die")
            .toArray();
        ...
        html.find(".dmgBtn-container-br button").click(async ev => {
            ...
            if (dmgElement.find(".red-extra-die").length > 0) {
                ...
                dmg = await this._applyCritDamage(
                    Number(dmg),
                    Number(critDmg),
                    dialogPosition
                );
            ...
            targetActors.forEach(actor => {
                actor.applyDamage(dmg, modifier);
            });
    ...
    async _applyCritDamage(dmg, critdmg, position) {
        const dialogResult = await new Promise(async (resolve, reject) => {
            ...
            const data = {
                title: i18n("brsw5e.chat.damageButtons.critPrompt.title"),
                content: "",
                buttons: {
                    one: {
                        icon: '<i class="fas fa-check"></i>',
                        label: i18n("brsw5e.chat.damageButtons.critPrompt.yes"),
                        callback: () => {
                            resolve(dmg + critdmg);
                        }
                    },
                    two: {
                        ...
                        label: i18n("brsw5e.chat.damageButtons.critPrompt.no"),
                        callback: () => {
                            resolve(dmg);
                        }
                    }
