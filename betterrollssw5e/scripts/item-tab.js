import { i18n, isAttack, isSave } from "./betterrollssw5e.js";
...
export async function addBetterRollsContent(app, protoHtml) {
    ...
    if (CONFIG.betterRollsSW5e.validItemTypes.indexOf(item.data.type) == -1) {
        return;
    }
    ...
    const betterRollsTabString = `<a class="item" data-group="primary" data-tab="betterRollsSW5e">${i18n(
        "Better Rolls"
    )}</a>`;
    tabSelector.append($(betterRollsTabString));

    const settingsContainer = html.find(".sheet-body");
    const altSecondaryEnabled = game.settings.get(
        "betterrollssw5e",
        "altSecondaryEnabled"
    );
    ...
    const betterRollsTemplate = await renderTemplate(
        "modules/betterrollssw5e/templates/red-item-options.html",
        {
            SW5E: CONFIG.SW5e,
            item,
            canConsume,
    ...
    if (
        game.settings.get("betterrollssw5e", "damageContextPlacement") !== "0"
    ) {
        const damageRolls = html
            .find(".tab.details .damage-parts .damage-part input")
            .toArray();
        // Placeholder is either "Context" or "Label" depending on system settings
        const placeholder = game.settings.get(
            "betterrollssw5e",
            "contextReplacesDamage"
        )
            ? "brsw5e.settings.label"
            : "brsw5e.settings.context";

        damageRolls.forEach((damageRoll, i) => {
            const contextField = $(
                `<input type="text" name="flags.betterRollsSW5e.quickDamage.context.${i}" value="${item
                    .data.flags.betterRollsSW5e.quickDamage.context[i] ||
                    ""}" placeholder="${i18n(
                    placeholder
                )}" data-dtype="String" style="margin-left:5px;">`
            );

            damageRoll.after(contextField[0]);

            // Add event listener to delete context when damage is deleted
            $($($(damageRoll)[0].parentElement).find(`a.delete-damage`)).click(
                async _ => {
                    const contextFlags = Object.values(
                        item.data.flags.betterRollsSW5e.quickDamage.context
                    );
        ...
        if (getProperty(item, "data.flags.betterRollsSW5e.quickOther")) {
            ...
            const otherContextField = $(
                `<input type="text" name="flags.betterRollsSW5e.quickOther.context" value="${item
                    .data.flags.betterRollsSW5e.quickOther.context ||
                    ""}" placeholder="${i18n(
                    placeholder
                )}" data-dtype="String" style="margin-left:5px;">`
            );
